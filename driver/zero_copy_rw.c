...
static struct sk_buff * _alloc_zcopy_zkb(struct net_device *dev, unsigned int length)
{
...
struct sk_buff * alloc_zcopy_zkb(struct net_device *dev, unsigned int length)
{
	u32 i = 0;
	...
	if(zcopy_rmem.started)
	{
		for(i = 0; i < zcopy_rmem.nics; ++i)
		{
			...
			{
				skb = _alloc_zcopy_zkb(dev, length);
#ifdef DEBUG
				if(skb)
				{
	...
	return alloc_regular_skb(dev, length);
}
EXPORT_SYMBOL(alloc_zcopy_zkb);

void free_zcopy_zkb(struct sk_buff * skb)
{
...
static inline int examine_sk_zcopy_skb(struct sk_zcopy * zkb)
{
...
void deliver_zcopy_zkb(struct sk_buff * skb)
{
	if(is_zcopy_packet(skb))
...
//another optional, make the phy as global in that way find the zkb will more faster.
static int alloc_phy_skb_ptr(struct zcopy_rw_mem * mem, u64 skb_count)
{
	struct phy_zkb * phy;
	u32 buckets = 0, i = 0;
	u64 total_ptr_size = skb_count * sizeof(struct sk_zcopy *);
...
static int alloc_init_rw_zcopy_zkb(struct zcopy_rw_mem * mem, int read)
{
	...
	if(alloc_phy_skb_ptr(mem, mem->zkb_count))
		return -1;
